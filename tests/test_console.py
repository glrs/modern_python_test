...
def test_main_succeeds(runner: CliRunner, mock_requests_get: Mock) -> None:
    """It returns with an status code of zero."""
    result = runner.invoke(console.main)
    assert result.exit_code == 0
...
def test_main_fails_on_request_error(
    ...
    """It returns with a non-zero status code if the request fails."""
...
def mock_wikipedia_random_page(mocker: MockFixture) -> Mock:
    ...
    return mocker.patch("modern_python_test.wikipedia.random_page")
...
def test_main_uses_specified_language(
    ...
    runner.invoke(console.main, ["--language=el"])
    mock_wikipedia_random_page.assert_called_with(language="el")
